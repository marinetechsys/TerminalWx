...
int GTerm::calc_color(int fg, int bg, int flags) { return (flags & 15) | (fg << 4) | (bg << 8); }

bool GTerm::changes_pending()
{
    int y;
    for (y = 0; y < height; y++)
    {
...
#ifdef GTERM_PC
                    if (mode_flags & PC)
                        DrawText((c >> 4) & 0xf, (c >> 8) & 0xf, c /*&15*/, start_x, y, x - start_x,
                                 text + yp + start_x);
                    else
#endif
                        DrawText((c >> 4) & 7, (c >> 8) & 7, c /*&15*/, start_x, y, x - start_x,
                                 text + yp + start_x);
                }
                else
                {
